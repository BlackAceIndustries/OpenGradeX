...
#define DIR1_RL_ENABLE  14  //PD4 //Dir1 for Cytron Dir, Both L and R enable for IBT2
#define PWM1_LPWM  12  //PD3 //PWM1 for Cytron PWM, Left PWM for IBT2
...
void SetOutput()
{
  if (isAutoActive && isCutting && isOGXConnected){    
    ...
    if (b_deltaDir == 1){ // Delta is Positive need to lower IMP RETRACT
      ...
      pwmDrive = map(PID_total, 0.0, -2300, 0 , 254);
      //ledcWrite(PWM_Channel, pwmDrive);
