...
uint16_t a2Port = 8888; // Antenna Port
uint16_t senderPort;
///Ip Addresses
IPAddress openGradeIP(192,168,1,226);   //OpenGradeX Server
IPAddress gradeControlIP(192,168,1,229);   // GradeControl Module IP
IPAddress a1IP(192,168,1,225);   // Antenna Module IP
IPAddress a2IP(192,168,1,225);   // Antenna Module IP
...
void loop(){ 
  ...
  currentTime = millis();  //  
  ArduinoOTA.handle();
  RelayGPSData2();  // Relay serial to UDP
...
bool SetupAntennaModule()
{
  ...
  digitalWrite(2, HIGH); delay(50); digitalWrite(2, LOW); delay(50); digitalWrite(2, HIGH); delay(50);
  digitalWrite(2, LOW); delay(50); digitalWrite(2, HIGH); delay(50); digitalWrite(2, LOW); delay(50);
...
{ 
  ...
  switch (_msgType){
    ...
    case Data:
        ...
        root["roll"] = antennaDataMsg.roll;
        root["pitch"] = antennaDataMsg.pitch;
        root["yaw"] = antennaDataMsg.yaw;
    ...
    case Diagnostic:
      ...
      UdpAntenna.beginPacket(a2IP,antennaPort);   //Initiate transmission of data
      UdpAntenna.print(payload);                 
      UdpAntenna.endPacket();  // Close communication   
...
{ 
  ...
  if (packetSize) {       // If we received a package
    ...
    if (modType == Antenna_Slave  && modID == 1){      
      if (msgType == 0)
      {
        ...
        connectMsg.readingId = root["readingId"].as<uint>();          
      ...
      if (msgType == int(Data))
      {
        //memcpy(&antennaDataMsg, packetBuffer, sizeof(antennaDataMsg));
        ...
        antennaDataMsg.readingId = root["readingId"].as<uint>();    


      }
      if (msgType == Settings)
      {
        //memcpy(&antennaSettingsMsg, packetBuffer, sizeof(antennaSettingsMsg));
        ...
        antennaSettingsMsg.readingId = root["readingId"].as<uint>(); 
      ...
      if (msgType == Diagnostic)
      {
        ...
        antennaFirmwareMsg.readingId = root["readingId"].as<uint>(); 
...
void ConnectToOGX() {
  WiFi.mode(WIFI_STA);
  WiFi.config(a2IP , gatewayIP, Subnet);
  WiFi.begin(ssid);
